...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while (len > 0) {   // 循环读取，直到读取到足够的数据
        ...
        if (is->audio_buf_index >= is->audio_buf_size) {
            //这里读取一个解码输出的帧，如果需要完成重采样的操作，把处理后的数据放到is->audio_buf
            ...
            if(!is->is_normal_playback_rate() && is->audio_buf) {
                ...
                // 2通道  目前只支持2通道的
                //因为格式不变，所以这个大小实际上只和输出采样点有关，别的参数都不变
...
int FFPlayer::audio_open(int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, AudioParams *audio_hw_params)
{
    ...
    //    SDL_OpenAudioDevice
    //打开音频设备
    //此时ffplay中的音频参数都被设置完了，包括sdl内部缓存大小
...
int FFPlayer::read_thread()
{
    ...
    if (seek_by_bytes < 0) {
        /*
    这行代码根据条件的结果来确定 seek_by_bytes 的值。如果输入格式支持时间戳不连续且不是 "ogg" 格式，
        那么 seek_by_bytes 被设置为 true（1）；否则，被设置为 false（0）。
*/
    ...
    while (1) {
        ...
        //如果两个队列加一起超过最大值或者单个队列有足够的包，暂停10ms等待解码线程消耗
...
double FFPlayer::compute_target_delay(double delay)
{
    ...
    if (get_master_sync_type() != AV_SYNC_VIDEO_MASTER) {
        ...
        sync_threshold = FFMAX(0.04, FFMIN(0.1, delay));
...
retry:
        if (frame_queue_nb_remaining(&pictq) == 0) {
            ...
            //上一帧和这一帧不是同个seriel，则frame_timer作废
            ...
            //             LOG(INFO) << "last_duraXZXXtion ......" << last_duration;
            ...
            //到这里目前时刻已经大于应该显示的时刻，所以更新frame_timer
            frame_timer += delay;
...
int Decoder::decoder_decode_frame(AVFrame * frame)
{
    ...
    for (;;) {
        ...
        do {
            ...
            // 2.2 如果还有pending的packet则使用它,是储存的pkt？？？
            if (packet_pending_) {
        ...
        if (pkt.data == flush_pkt.data) {//
            ...
            if (avctx_->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                ...
                //解码器内部缓存满了，现在无法发数据，保存在解码器内部设置pending标志位为1
...
int Decoder::audio_thread(void *arg)
{
    ...
    do {
        // 获取缓存情况，返回的是音频包队列剩余包总共播放时间，包的数量和大小总和
